...
- `.claude/skills`: symlinked to `.agents/skills` by the apply script so Claude Code can invoke the same workflows without duplicate files.
...
The apply script creates `.claude/skills -> ../.agents/skills`. This avoids maintaining two source copies while still giving Claude Code its expected project skill layout.
